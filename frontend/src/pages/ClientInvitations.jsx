...
const ClientInvitations = ({ user }) => {
  ...
  const fetchInvitedProjects = async () => {
    try {
      setLoading(true);
      // Fetch invitations that are pending OR accepted
      ...
      const allInvitations = [
        ...
        ...acceptedSnapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }))
      ];
      
      // Filter out invitations where the contract has been signed (project is active)
      const filteredInvitations = [];
      for (const invitation of allInvitations) {
        const projectDoc = await getDoc(doc(db, "projects", invitation.projectId));
        if (projectDoc.exists()) {
          const projectStatus = projectDoc.data().status;
          // Only include if project is still pending_contract or pending_invitation
          if (projectStatus === "pending_contract" || projectStatus === "pending_invitation") {
            filteredInvitations.push(invitation);
          }
        }
      }
      
      setInvitedProjects(filteredInvitations);
      ...
      for (const invitation of filteredInvitations) {
        try {
          const projectDoc = await getDoc(
            doc(db, "projects", invitation.projectId),
          );
          ...
          const freelancerDoc = await getDoc(
            doc(db, "users", invitation.freelancerId),
          );
  ...
  const handleAcceptInvitation = async (invitation) => {
    try {
      ...
      if (result.success) {
        await fetchInvitedProjects();
        // Redirect to contract review
        if (result.data?.contractId) {
          navigate(`/contracts/${result.data.contractId}/review`);
        } else {
          alert(
            "Contract was created but couldn't navigate to it. Please check your dashboard.",
          );
        }
      } else if (result.error === "Invitation already accepted") {
        // If already accepted, fetch the contract and redirect
        try {
          ...
          if (!contractSnapshot.empty) {
            ...
            alert("Contract not found. Please contact support.");
          }
        } catch (err) {
          console.error("Error fetching contract:", err);
          alert("Could not find contract. Please try again.");
        }
      } else {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            You have {invitedProjects.length} pending invitation
            {invitedProjects.length !== 1 ? "s" : ""}
          </p>
        </div>
                    ...
                    {project && (
                      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                        ...
                        <div className="flex items-center text-sm">
                          ...
                          <span
                            className={`ml-2 px-2 py-1 rounded-full text-xs font-medium ${getPriorityColor(project.priority)}`}
                          >
                        ...
                        <div className="flex items-center text-sm">
                          ...
                          <span
                            className={`ml-2 px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(project.status)}`}
                          >
