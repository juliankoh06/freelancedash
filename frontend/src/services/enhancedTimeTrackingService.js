...
class EnhancedTimeTrackingService {
  ...
  async startTracking(taskId, projectId, userId) {
    try {
      const url = `http://localhost:5000/api/time-tracking/start/${taskId}`;
  ...
  async stopTracking(taskId, notes = "") {
    try {
      const url = `http://localhost:5000/api/time-tracking/stop/${taskId}`;
...
const enhancedTimeTrackingService = new EnhancedTimeTrackingService();
export default enhancedTimeTrackingService;
